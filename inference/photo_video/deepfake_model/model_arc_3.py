...
class FakeCatcher(nn.Module):
    def __init__(self, device):
        ...
        self.conv_to_vector3 = nn.Sequential(
            nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=True),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),
            nn.Conv2d(16, 32, kernel_size=3, stride=1, padding=1, bias=True),
    ...
    def forward(self, x):
        ...
        real_out = self.conv_to_vector3(x)

        conv_out = torch.flatten(conv_out, 1)
        fft_out = torch.flatten(fft_out, 1)
        real_out = torch.flatten(real_out, 1)
        combined_out = torch.cat((conv_out, fft_out, real_out), dim=1)
